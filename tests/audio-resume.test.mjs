...
await test('Safari audio activation and interruption lifecycle', async (t) => {
  ...
  function setup() {
    ...
    return { audio, ready, ctx: current };
  }
  await t.test('first tap and bot speech survive delayed resume exactly once', async () => {
    const { audio, ready, ctx } = setup();
    audio.play('tap');
    audio.speak('positive', 1);
  ...
  await t.test('queue is bounded to eight action cues', async () => {
    const { audio, ready, ctx } = setup();
    for (let i = 0; i < 20; i++) audio.play('place');
    ctx.finishResume();
    await ready;
  ...
  await t.test('saved SFX off remains off', async () => {
    saved.set('vietnamtown-sound', 'off');
    const audio = createGameAudio();
    assert.equal(audio.getSettings().sfx, false);
    audio.destroy();
  });
});
