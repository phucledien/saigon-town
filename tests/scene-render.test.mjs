...
import { BLOCK_LAYOUTS, PLOT_LAYOUT } from '../src/board-layout.ts';
import { cameraFor, dragCamera } from '../src/camera.ts';
import { zodiacImages } from '../src/zodiac.ts';
...
test('plot taps, shop builds and phase changes never replace the running street scene', () => {
  ...
  function node(selector) {
    ...
    const result = {
      ...
      set innerHTML(value) {
        assert.ok(
          !protectedNodes.has(selector) || (selector === '#app' && mounts === 0),
          `${selector} would disconnect the running scene`,
        );
        html = value;
        if (selector === '#app') {
          mounts++;
          for (const match of value.matchAll(/class="([^"]+)"/g))
            for (const cls of match[1].split(' '))
              if (!nodes.has('.' + cls)) nodes.set('.' + cls, node('.' + cls));
          sceneRefs = [
            '.game-shell',
            '.map-viewport',
            '.game-world',
  ...
  const state = engine.newGame(() => 0.31),
    frames = [];
  const ctx = vm.createContext({
    ...
    get: (selector) => nodes.get(selector) || null,
    state,
    language: 'vi',
    view: 'play',
    menuOpen: false,
    ...
    TOKEN_COLORS: ['green', 'gold', 'red', 'blue'],
    money: (n) => `${n} đ`,
    tr: (en, vi) => vi,
    escape: (s) => s,
    document: {
  ...
  vm.runInContext(
    source.slice(source.indexOf('function sprite('), source.indexOf('function gameMenu(')),
    ctx,
  );
  function refresh() {
    ctx.render();
  ...
  engine.claimPlots(state, offered, () => 0.2);
  ctx.chosen = [];
  refresh();
  assert.match(nodes.get('.game-hud-bottom').innerHTML, /Trả giá/);
  engine.beginBuild(state);
  refresh();
  ctx.selectedShop = state.players[0].tiles.findIndex((n) => n > 0);
  ctx.rackOpen = true;
  refresh();
  ...
  engine.placeShop(state, plot, ctx.selectedShop);
  ctx.selectedLot = plot;
  refresh();
  assert.match(nodes.get('.plot-layer').innerHTML, /built-shop/);
  engine.finishYear(state);
  refresh();
  engine.nextYear(state);
  refresh();
